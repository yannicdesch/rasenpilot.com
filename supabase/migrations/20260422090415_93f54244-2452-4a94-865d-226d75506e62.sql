...
BEGIN
  ...
  -- Find the best scoring claimed job
  SELECT 
    id,
    COALESCE((result->>'score')::INTEGER, 0),
    result->>'image_url'
  INTO best_job_id, best_score, best_image
  ...
  IF best_score IS NOT NULL AND best_score > 0 THEN
    ...
    -- Use the registered first name; fallback to anonymized initial from email
    IF user_first_name IS NOT NULL AND length(trim(user_first_name)) > 0 THEN
      display_name := trim(user_first_name);
    ELSE
      display_name := upper(substr(p_email, 1, 1)) || '***';
    END IF;

    -- Prefer ZIP from the analysis job metadata (entered during upload)
    SELECT metadata->>'zipCode' INTO user_zip
    FROM public.analysis_jobs
    WHERE id = best_job_id;

    -- Fallback: lawn profile
    IF user_zip IS NULL OR length(user_zip) = 0 OR user_zip = '10115' THEN
      ...
      LIMIT 1;
    ELSE
      SELECT grass_type, lawn_size 
      INTO user_grass, user_size
      FROM public.lawn_profiles 
      WHERE user_id = p_user_id 
      ORDER BY created_at DESC 
      LIMIT 1;
